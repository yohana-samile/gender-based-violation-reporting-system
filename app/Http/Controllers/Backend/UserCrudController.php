...
use App\Http\Requests\Backend\User\CreateUserRequest as StoreRequest;
use App\Http\Requests\Backend\User\UpdateUserRequest as UpdateRequest;
...
class UserCrudController extends Controller
{
    ...
    public function store(StoreRequest $request)
    {
        ...
        try {
            $response = $this->user_repository->store($input);
            if ($response === false) {
                return response()->json([
                    'status' => 500,
                    'message' => "Fail to register admin user",
                ]);
            }
    ...
    public function deleteUser($user)
    {
        ...
        $destination = route('backend.user');
        DB::beginTransaction();
    ...
    public function profile($userId)
    {
        ...
        return view('pages.backend.user.profile', ['user' => $user]);
    }

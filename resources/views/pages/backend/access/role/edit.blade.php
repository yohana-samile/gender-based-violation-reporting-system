        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" value="{{ $role->name }}" required placeholder="" autocomplete="off"
                           class="w-full mt-1 rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-blue-500 focus:border-blue-500">
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="description" id="description" value="{{ $role->description }}" placeholder="" autocomplete="off"
                           class="w-full mt-1 rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-blue-500 focus:border-blue-500">
                    @error('description')
                    <span class="text-sm text-red-500">{{ $message }}</span>
                    @enderror
            ...
            <div class="mt-8">
                {{ Html::labels('permission_id', __('Permission'), ['class' => 'font-semibold text-gray-800 dark:text-gray-100']) }}
                <div class="overflow-x-auto mt-4 border border-gray-200 dark:border-gray-600 rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-600 bg-white dark:bg-gray-800 text-sm permissionTable">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex flex-wrap gap-3">
                                            @forelse($groupPermissions as $permission)
                                                <label class="flex items-center space-x-2">
                                                    <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" class="permissioncheckbox rounded"
                                                        {{ in_array($permission->id, $rolePermissions) ? 'checked' : '' }}>
                                                    <span>{{ $permission->display_name }}</span>
                                                </label>
                                            @empty
                                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('no permission group') }}</p>
                                            @endforelse
                                        </div>
                                    </td>
                                </tr>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    <label class="inline-flex items-center space-x-2">
                        <input type="checkbox" name="isactive" class="form-checkbox rounded text-blue-600 dark:bg-gray-700 dark:border-gray-600" id="isactive" value="1" @if($role->isactive) checked @endif>
                        <span class="text-gray-700 dark:text-gray-300">{{ __('is_active') }}</span>
                    </label>
                </div>

                <div>
                    <label class="inline-flex items-center space-x-2">
                        <input type="checkbox" name="isadmin" class="form-checkbox rounded text-blue-600 dark:bg-gray-700 dark:border-gray-600" id="isadmin" value="1" @if($role->isadmin) checked @endif>
                        <span class="text-gray-700 dark:text-gray-300">{{ __('is_admin') }}</span>
                    </label>
                </div>
            </div>

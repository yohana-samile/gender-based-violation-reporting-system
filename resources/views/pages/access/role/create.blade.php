        ...
        <div class="space-y-6">
            <div class="bg-white shadow-md rounded-lg p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        {{ Html::labels('name', __('name'), ['class' =>'required_asterik block text-sm font-medium text-gray-700']) }}
                        {{ Html::texts('name', null, ['class'=>'mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm', 'id' => 'name', 'autocomplete' => 'off']) }}
                        {!! $errors->first('name', '<span class="text-red-500 text-sm">:message</span>') !!}
                    </div>

                    <div>
                        {{ Html::labels('description', __('descriptions'), ['class' =>'required_asterik block text-sm font-medium text-gray-700']) }}
                        {{ Html::texts('description', null, ['class'=>'mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm', 'id' => 'description', 'autocomplete' => 'off']) }}
                        {!! $errors->first('description', '<span class="text-red-500 text-sm">:message</span>') !!}
                    </div>
                </div>

                <div class="mt-6">
                    {{ Html::labels('permission_id', __('Permission'), ['class' =>'block text-sm font-medium text-gray-700']) }}
                    <table class="min-w-full text-sm mt-3 border bg-white shadow rounded-lg permissionTable">
                                        ...
                                        @forelse($groupPermissions as $permission)
                                            <label class="inline-flex items-center text-gray-700">
                                                <input type="checkbox" name="permissions[]"
                                                       value="{{ $permission->id }}" class="permissioncheckbox mr-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div class="flex items-center space-x-2">
                        ...
                        {{ Html::labels('isactive', __('is_active'), ['class' => 'text-sm font-medium text-gray-700']) }}
                    </div>
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        {{ Html::labels('isadmin', __('is_admin'), ['class' => 'text-sm font-medium text-gray-700']) }}
                    </div>
                </div>

...
@section('content')
    <div class="bg-white shadow rounded-lg p-6">
        <div class="mb-4 border-b border-gray-200">
            <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" role="tablist">
                <li class="mr-2" role="presentation">
                    <a href="#general_tab"
                       class="inline-block p-4 border-b-2 border-blue-500 text-blue-600 active"
        ...
        <div class="tab-content text-gray-700">
                ...
                <div class="mb-4 flex justify-end">
                    @if(access()->allow('manage_role_and_permissions'))
                        <a href="{{ route('backend.role.edit', $role->uid) }}"
                           class="inline-flex items-center px-3 py-1 text-sm font-medium text-white bg-blue-600 rounded hover:bg-blue-700 mr-2">
                        ...
                        @if($role->checkIfCanBeDeleted())
                            {!! link_to_route('backend.role.delete', trans('label.crud.delete'),
                                [$role->uid], [
                                    ...
                                    'class' => 'inline-flex items-center px-3 py-1 text-sm font-medium text-white bg-red-600 rounded hover:bg-red-700 mr-2'
                                ])
                            !!}
                        @endif
                    @endif
                    ...
                    <a href="{{ route('backend.role.index') }}"
                       class="inline-flex items-center px-3 py-1 text-sm font-medium text-white bg-red-500 rounded hover:bg-gray-700">
                        <i class="ri-close mr-2 text-white"></i> {{ __('Close') }}
                    </a>
                </div>

                @include('pages.access.role.profile.includes.general_info')
            </div>
